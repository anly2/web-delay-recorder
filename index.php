...
if(isset($_REQUEST['valids'])){
   ...
   foreach($_REQUEST['valids'] as $i=>$avg)
      $_SESSION['valids'][$i] = $avg;
...
if(isset($_REQUEST['js'])){
   ...
   if($_REQUEST['js']=='dr_main'){
      ...
      echo "\n";
      echo '   document.writeln(\'<input type="password" id="\'+this.id+\'" name="\'+name+\'" onkeydown="if(!window.event) window.event = event; this.dr.effect();"/><b onclick="this.previousSibling.dr.reset();">R</b>\');'."\n";
   ...
   if($_REQUEST['js']=='dr_pack'){
      ...
      echo '   for(i in valids)'."\n";
      echo '      call(\'?valids[\'+i+\'][0]=\'+valids[i].from+\'&valids[\'+i+\'][1]=\'+valids[i].to, true);'."\n";
      echo '}'."\n";
   }
   if($_REQUEST['js']=='dr_fun' ){
      echo 'delayRecorder.prototype.announceTarget   = false;'."\n";
      echo 'delayRecorder.prototype.announceDetailed = false;'."\n";
      echo "\n";
...
echo "\n";
echo '   <script type="text/javascript" src="?js=dr_main"></script>'."\n";
echo '   <script type="text/javascript" src="?js=dr_pack"></script>'."\n";
echo '   <script type="text/javascript" src="?js=dr_fun" ></script>'."\n";
echo '</head>'."\n";
echo '<body>'."\n";
echo '   <form action="?" method="post" onsubmit=" if(!valids || passAdv.verify(true)){ passAdv.calculate(); passAdv.pack(); return true;}else{ this.passed.value=\'false\'; return false; }">'."\n";
